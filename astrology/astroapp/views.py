...
def zodiac(request):
    if not check_session(request):
        return redirect('home')  # Redirect to home if session is null
    ...
    if request.method == 'POST':
        ...
        # Determine the astrological sign based on the month and day
        if month == 'december':
        ...
        if astro_sign != 'Invalid Month':
            # Save the zodiac sign in the database
            zodiacsign = astro_sign
            zodiacsave = zodiacdb(birth_month=month, birth_day=day, place_of_birth=place, astro_sign=zodiacsign)
            zodiacsave.save()  # Save the new record
            return render(request, 'zodiac.html', {'uname': uname, 'astro_sign': zodiacsign})
...
def updatepwd(request):
    ...
    flag = registration.objects.filter(Q(password=opwd) & Q(name=auname))
...
def logout(request):
    # Clear and flush the session to ensure a complete logout
    uname = request.session["uname"]
